...
전국 17개 시도교육청이 공개한 **정책계획(기본계획·추진계획·운영계획 등)을 한 곳에서 통합검색**하고,
...
data/
  ...
  budget.json                  17개 교육청 세출예산(계층 포함)
config/
...
**1단계 — 게시판 연결(메타데이터 색인)** ✅
- [x] 17개 시도교육청 게시판 수집기 연결 (191개 게시판)
- [x] 세출예산 검색·비교
...
> **실시간 조회는 하지 않음** — 검색할 때마다 17개 사이트를 찌르면 부하·지연이 커서, 주기적 스냅샷 방식을 유지한다.
